...
def ai(queue):
    ...
    objectTracker.setDetectionLabelsToTrack([14])  # track #0: aeroplane voc; #14:person voc
    ...
    with dai.Device(pipeline,maxUsbSpeed=dai.UsbSpeed.HIGH) as device:
        ...
        fps = 0
        color = (0,0,0)
        global state,flag_track,cx,altCur,altTar,cy,cyt,cyn
        cx = 320 # pixels
        cy = 200
        ID = 0
        cyn = 1 # normalized cy
    ...
    while True:
        ...
        if (vehicle.mode.name=='LOITER'):    
            ...
            while event.is_set():
                ...
                if state == "fly":
                    if flag_track:
                        print('cx,cy:',cx,cyn,file=f) # log
                        speed_yaw,speed_pitch = traceDrone(cx,cyn)
...
def sav(queue):
    ...
    # might causing freezing video: solve by separate for saving video
    ...
    if not stream_out.isOpened():
        ...
        print("INFO: On your H12 Pro, connect to: rtsp://<YOUR_RPI_IP>:554/stream0")
    item = None
    while True:
        item = queue.get()  # Get frame from the AI processing thread
