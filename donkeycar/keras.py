...
class KerasLinear(KerasPilot):
    ...
    opencv blob,resize(width,height)<=> donkeycar input_shape(height,width)
    '''
...
def default_categorical(input_shape=(120, 160, 3), roi_crop=(0, 0)):
    ...
    a = b*c*d
    x = Permute([1, 2, 3])(x)
    x = Reshape((int(a),))(x) # convert dim -> int                                        # Flatten to 1D (Fully connected)
